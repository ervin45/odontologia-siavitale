...
public class BancoController {
	...
	@FXML
	private TableView<Banco> Tabla;
	
	@FXML
	private TableColumn<Banco, String> ColCodigo;
	
	@FXML
	private TableColumn<Banco, String> ColDescripcion;
	...
	private ObservableList<Banco> tablaarray = FXCollections.observableArrayList();
	
	private Banco actual = null;
...
//	}
	...
	private void initialize() {
		...
		ColCodigo.setCellValueFactory(new PropertyValueFactory<Banco, String>("Codigo"));
		ColDescripcion.setCellValueFactory(new PropertyValueFactory<Banco, String>("Descripcion"));
		
		Tabla.setColumnResizePolicy(TableView.UNCONSTRAINED_RESIZE_POLICY);
		
		mostrarTablaDetalles(null);
		
		Tabla.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Banco>(){

			@Override
			public void changed(ObservableValue<? extends Banco> arg0,
					Banco arg1, Banco arg2) {
	...
	private void cargarTabla() {
		...
		try {
			...
			Query queryResult = sesion.createQuery("from Banco");
	...
	private void mostrarTablaDetalles(Banco mostrar) {
		if (mostrar != null)
		{
...
//			tfCodigo.setText("");
	...
	private void clickbotonBorrar()
	{
		...
		Banco Dborrar = Tabla.getSelectionModel().getSelectedItem();
...
//			tfStatus.setEditable(false);				
	...
	{
		Banco modi = Tabla.getSelectionModel().getSelectedItem();
		actual = modi;
		System.out.println("Entroó en Botón Borrar!!!!");
		if (modi != null)
		{
...
//			actual.setCodigo(tfCodigo.getText());
	...
	{
		Banco sel = Tabla.getSelectionModel().getSelectedItem();
		actual = sel;
		mostrarTablaDetalles(actual);
		modifica = false;
		edicion(false);
