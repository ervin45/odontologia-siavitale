...
public class ConfiguraInventarioController {

    private ObservableList<AArea> areaarray = FXCollections.observableArrayList(); 
...
//    	initialize3();
    ...
    {
    	ColDescripcion.setCellValueFactory(new PropertyValueFactory<AArea, String>("Descripcion"));
//    	ColComentario.setCellValueFactory(new PropertyValueFactory<AArea, String>("Comentario"));
    	cargaAArea();
    	modoconsulta();
    	...
    	Tabla.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<AArea>()
    			{

					@Override
					public void changed(ObservableValue<? extends AArea> arg0,
							AArea arg1, AArea arg2) {
						actual = arg2;
						muestraAArea(actual);
					}
    ...
    private void cargaAArea()
    {
    	...
    	try
    	{
    		...
    		Query quee = sees.createQuery("from AArea");
    		areaarray = FXCollections.observableArrayList(quee.list());
    		sees.getTransaction().commit();
    	} catch (HibernateException ee)
    	{
    		ee.printStackTrace();
    	}
    	Tabla.setItems(areaarray);
    }

    private void grabaAArea(AArea actual2)
    {
    	try
    	{
    		Session sss = Principal.fabrica.getCurrentSession();
    		sss.getTransaction().begin();
    		sss.saveOrUpdate(actual2);
    ...
    private void muestraAArea(AArea actual2)
    {
    	actual = actual2;
    	if (actual==null)
    	{
    	...
    	{
    		tfDescripcion.setText(actual2.getDescripcion());
        	tfComentario.setText(actual2.getComentario());
    	}
    }
...
//    	Tabla.edit(0, ColDescripcion);
    ...
    void clickbtBorra(ActionEvent event) {
    	...
    	{
    		actual = (AArea) Tabla.getSelectionModel().getSelectedItem();
//        	modoedicion();
