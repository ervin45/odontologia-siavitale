...
public class CuentaContableController {
	...
	@FXML
	private TableView<CuentaContable> Tabla;
	
	@FXML
	private TableColumn<CuentaContable, String> ColCodigo;
	
	@FXML
	private TableColumn<CuentaContable, String> ColDescripcion;
	...
	private ObservableList<CuentaContable> tablaarray = FXCollections.observableArrayList();
	
	private CuentaContable actual = null;
...
//	}
	...
	private void initialize() {
		...
		titulo.setText("Cuenta Contable");
		
		ColCodigo.setCellValueFactory(new PropertyValueFactory<CuentaContable, String>("Codigo"));
		ColDescripcion.setCellValueFactory(new PropertyValueFactory<CuentaContable, String>("Descripcion"));
		...
		Tabla.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<CuentaContable>(){
			...
			@Override
			public void changed(ObservableValue<? extends CuentaContable> arg0,
					CuentaContable arg1, CuentaContable arg2) {
	...
	private void cargarTabla() {
		...
		try {
			...
			Query queryResult = sesion.createQuery("from CuentaContable");
	...
	private void mostrarTablaDetalles(CuentaContable mostrar) {
		if (mostrar != null)
		{
...
//			tfCodigo.setText("");
	...
	private void clickbotonBorrar()
	{
		...
		CuentaContable Dborrar = Tabla.getSelectionModel().getSelectedItem();
...
//			tfStatus.setEditable(false);				
	...
	{
		CuentaContable modi = Tabla.getSelectionModel().getSelectedItem();
		actual = modi;
		System.out.println("Entroó en Botón Borrar!!!!");
		if (modi != null)
		{
...
//			actual.setCodigo(tfCodigo.getText());
	...
	{
		CuentaContable sel = Tabla.getSelectionModel().getSelectedItem();
		actual = sel;
		mostrarTablaDetalles(actual);
		modifica = false;
		edicion(false);
