...
public class CentrodeCostoController {
	...
	@FXML
	private TableView<CentrodeCosto> Tabla;
	
	@FXML
	private TableColumn<CentrodeCosto, String> ColCodigo;
	
	@FXML
	private TableColumn<CentrodeCosto, String> ColDescripcion;
	...
	private ObservableList<CentrodeCosto> tablaarray = FXCollections.observableArrayList();
	
	private CentrodeCosto actual = null;
...
//	}
	...
	private void initialize() {
		...
		titulo.setText("Descripción General");
		
		ColCodigo.setCellValueFactory(new PropertyValueFactory<CentrodeCosto, String>("Codigo"));
		ColDescripcion.setCellValueFactory(new PropertyValueFactory<CentrodeCosto, String>("Descripcion"));
		
		Tabla.setColumnResizePolicy(TableView.UNCONSTRAINED_RESIZE_POLICY);
		
		mostrarTablaDetalles(null);
		
		Tabla.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<CentrodeCosto>(){

			@Override
			public void changed(ObservableValue<? extends CentrodeCosto> arg0,
					CentrodeCosto arg1, CentrodeCosto arg2) {
	...
	private void cargarTabla() {
		...
		try {
			...
			Query queryResult = sesion.createQuery("from CentrodeCosto");
	...
	private void mostrarTablaDetalles(CentrodeCosto mostrar) {
		if (mostrar != null)
		{
...
//			tfCodigo.setText("");
	...
	private void clickbotonBorrar()
	{
		...
		CentrodeCosto Dborrar = Tabla.getSelectionModel().getSelectedItem();
...
//			tfStatus.setEditable(false);				
	...
	{
		CentrodeCosto modi = Tabla.getSelectionModel().getSelectedItem();
		actual = modi;
		System.out.println("Entroó en Botón Borrar!!!!");
		if (modi != null)
		{
...
//			actual.setCodigo(tfCodigo.getText());
	...
	{
		CentrodeCosto sel = Tabla.getSelectionModel().getSelectedItem();
		actual = sel;
		mostrarTablaDetalles(actual);
		modifica = false;
		edicion(false);
