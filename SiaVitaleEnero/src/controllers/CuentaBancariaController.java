...
public class CuentaBancariaController {
	...
	@FXML
	private TableView<CuentaBancaria> Tabla;
	
	@FXML
	private TableColumn<CuentaBancaria, String> ColCodigo;
	
	@FXML
	private TableColumn<CuentaBancaria, String> ColDescripcion;
	...
	private ObservableList<CuentaBancaria> tablaarray = FXCollections.observableArrayList();
	
	private CuentaBancaria actual = null;

	private Boolean modifica = false;

//	public static void mostrarCuentaBancaria() {
//		
//		try {
...
//	}
	...
	private void initialize() {
		...
		titulo.setText("Cuenta Bancaria");
		
		ColCodigo.setCellValueFactory(new PropertyValueFactory<CuentaBancaria, String>("Codigo"));
		ColDescripcion.setCellValueFactory(new PropertyValueFactory<CuentaBancaria, String>("Descripcion"));
		
		Tabla.setColumnResizePolicy(TableView.UNCONSTRAINED_RESIZE_POLICY);
		
		mostrarTablaDetalles(null);
		
		Tabla.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<CuentaBancaria>(){

			@Override
			public void changed(ObservableValue<? extends CuentaBancaria> arg0,
					CuentaBancaria arg1, CuentaBancaria arg2) {
	...
	private void cargarTabla() {
		...
		try {
			...
			Query queryResult = sesion.createQuery("from CuentaBancaria");
	...
	private void mostrarTablaDetalles(CuentaBancaria mostrar) {
		if (mostrar != null)
		{
...
//			tfCodigo.setText("");
	...
	private void clickbotonBorrar()
	{
		...
		CuentaBancaria Dborrar = Tabla.getSelectionModel().getSelectedItem();
...
//			tfStatus.setEditable(false);				
	...
	{
		CuentaBancaria modi = Tabla.getSelectionModel().getSelectedItem();
		actual = modi;
		System.out.println("Entroó en Botón Borrar!!!!");
		if (modi != null)
		{
...
//			actual.setCodigo(tfCodigo.getText());
	...
	{
		CuentaBancaria sel = Tabla.getSelectionModel().getSelectedItem();
		actual = sel;
		mostrarTablaDetalles(actual);
		modifica = false;
		edicion(false);
